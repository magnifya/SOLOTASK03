"""Tests for signed finality-history fork location.
...
* the first in-order locator naming a **confirmed** main-chain block
  anchors the signed page (a pending tip never matches), a locator list
  with no confirmed main-chain hit is 409, and the success body reuses the
  fixed ``anchor, finalities, next, head`` shape of GET
  /v1/chain/finalities with the matched locator as the anchor;
...
class FinalityLocatorFixture(unittest.TestCase):
    ...
    def re_sign(self, credential: dict) -> None:
...
class FinalityLocatorServiceTests(FinalityLocatorFixture):
    def test_locates_first_matching_locator_in_order(self) -> None:
        ...
        # whose second item is a confirmed main-chain block.
        payload = {
            "locators": [
        ...
        # The credentials run ascending strictly after the height-2 anchor;
        # the pending tip at height 4 is never finalized.
        self.assertEqual(
    ...
    def test_first_in_order_match_wins_not_a_deeper_one(self) -> None:
        payload = {"locators": [self.loc(2), self.loc(0)]}
        status, body = self.locate(payload)
        self.assertEqual(status, 200, body)
        self.assertEqual(body["anchor"], self.loc(2))

    def test_pending_tip_locator_never_matches(self) -> None:
        # The pending tip at height 4 is not confirmed: a locator naming it
        # is skipped and the next confirmed locator anchors.
        payload = {"locators": [self.loc(4), self.loc(2)]}
        status, body = self.locate(payload)
        self.assertEqual(status, 200, body)
        self.assertEqual(body["anchor"], self.loc(2))
        # Alone, the pending locator is no match at all.
    ...
    def test_anchor_at_finalized_head_is_empty_signed_page(self) -> None:
    ...
    def test_no_confirmed_main_chain_match_is_409(self) -> None:
        cases = [
            ...
            # A locator above the tip, with a lower fork after.
...
class FinalityLocatorVerifyTests(FinalityLocatorFixture):
    ...
    def verify(self, pages, locators, tip_hash=_UNSET, trust=_UNSET) -> dict:
    ...
    def test_verifies_genuine_page_and_reports_index(self) -> None:
        ...
        self.assertEqual(result["head"], page["head"])
        self.assertEqual(result["verified_block_hashes"], [self.h(3)])

    def test_multi_page_history(self) -> None:
        pages = self.paged(limit=1)
        self.assertEqual(len(pages), 3)
        result = self.verify(pages, [self.loc(4, "f" * 64), self.loc(0)])
        self.assertTrue(result["ok"], result)
        self.assertEqual(result["matched_index"], 1)
    ...
    def test_index_zero_and_empty_page(self) -> None:
        # The finalized head anchors an empty single page.
        page = self.finalities_page(3, self.h(3))
        self.assertEqual(page["finalities"], [])
        result = self.verify([page], [self.loc(3)])
        self.assertTrue(result["ok"], result)
        self.assertEqual(result["matched_index"], 0)
        self.assertEqual(result["verified_block_hashes"], [])

    def test_input_errors(self) -> None:
        page = self.finalities_page(0, self.genesis_hash)
        ...
        # Malformed pages stay input even with good locators.
        self.assertEqual(
            self.verify([], [self.loc(0)]),
            {"ok": False, "error": ERR_INPUT},
        )
        self.assertEqual(
            self.verify("nope", [self.loc(0)]),
            {"ok": False, "error": ERR_INPUT},
        )
    ...
    def test_auth_errors(self) -> None:
        ...
        tampered = copy.deepcopy(page)
        tampered["head"]["auth"]["signature"] = "0" * 128
        self.assertEqual(
            self.verify([tampered], locators), {"ok": False, "error": ERR_AUTH}
        )
    ...
    def test_integrity_errors(self) -> None:
        ...
        result = self.verify([page], [{"height": 0, "block_hash": "a" * 64}])
        ...
        self.assertEqual(result, {"ok": False, "error": ERR_INTEGRITY})
        # A tampered finalized hash, re-signed (auth passes), anchor still
        # listed: the broken descriptor is integrity.
        tampered = copy.deepcopy(page)
        tampered["finalities"][0]["tip"]["status"] = "pending"
        self.re_sign(tampered["finalities"][0])
        result = self.verify([tampered], [self.loc(0)])
        self.assertEqual(result, {"ok": False, "error": ERR_INTEGRITY})
        # A broken pagination chain is integrity.
        pages = self.paged(limit=1)
        broken = [pages[0], pages[2]]
        result = self.verify(broken, [self.loc(0)])
        self.assertEqual(result, {"ok": False, "error": ERR_INTEGRITY})

    def test_membership_uses_exact_closed_document(self) -> None:
        # An equal document at the right position matches even if it is a
        # distinct object.
        page = self.finalities_page(1, self.h(1))
...
class FinalityLocatorHttpTests(unittest.TestCase):
    ...
    def test_locate_http_success_and_order(self) -> None:
        ...
        # The wire order is the contract order, not alphabetical.
