...
history-export subcommands and the offline header-checkpoint
header-locators subcommand.
...
def cmd_sync_export(args: argparse.Namespace) -> int:
    # Export one received sync candidate by its idempotency key. The success
...
def cmd_sync_range_export(args: argparse.Namespace) -> int:
    # Export one received incremental range delivery by its idempotency key.
    # The success document (source, request_id, mode, expires_at, anchor,
    # blocks, tip, attestation) has a contract-fixed key order, so it is
    # printed in insertion order rather than alphabetically (error bodies are
    # single-key).
    query = urllib.parse.urlencode(
        {
            "source": args.source,
...
def cmd_sync_range_attested(args: argparse.Namespace) -> int:
    try:
        ...
        return _emit(400, {"error": f"invalid JSON range document: {exc}"})
    # Same range document as sync-range: {"anchor", "blocks"[, "tip"]}; a
    # GET /v1/chain/range page may be piped verbatim (its extra canonical/
    # next_height fields are ignored) and the tip is derived from the final
    # delivered block when omitted.
    if not isinstance(document, dict):
        return _emit(400, {"error": "range document must be a JSON object"})
    anchor = document.get("anchor")
    blocks = document.get("blocks")
    ...
    if tip is None and blocks:
        ...
        # plain range page (which carries no tip) can be pushed verbatim.
        last = blocks[-1]
        if isinstance(last, dict):
            anchor_height = anchor.get("height")
            length = (
    ...
    # The signature is over the canonical {domain, source, request_id,
    # expires_at, anchor, blocks, tip} message; the 64-lowercase-hex seed
    # signs the raw SHA-256 digest, producing a 128-lowercase-hex signature.
    signed = {
...
def _decimal_int(value: str) -> int | None:
    """Parse a decimal CLI integer, or None for anything unparseable.

    Kept manual (rather than an argparse ``type=int``) so a malformed option
    reports the command's JSON ``input`` envelope and exit 1 instead of an
...
def _emit_result_document(body: dict) -> int:
    ...
    A success is the document itself (a page or a ``{root, records, head}``
    log) with no ``ok`` key; only a failure carries ``{"ok": false,
    "error"}``. Insertion order is kept so declared key orders survive.
    """
    print(json.dumps(body, sort_keys=False, ensure_ascii=False))
    return 1 if body.get("ok") is False else 0


def _input_failure() -> int:
    return _emit_result_document({"ok": False, "error": "input"})


def cmd_history_trust(args: argparse.Namespace) -> int:
    """Read or update the durable checkpoint-history signer log.

    No update options means read; an update requires all four of
    ``--root-key/--at/--key/--status``. Prints the ``{root, records, head}``
    document as one contract-ordered JSON line, exit 0/1 on success/failure.
    """
    ...
    at_value = _decimal_int(args.at) if args.at is not None else None
    if args.at is not None and at_value is None:
        return _input_failure()
    body = history_trust(
        args.path, args.root_key, at_value, args.key, args.status
    )
    return _emit_result_document(body)


def cmd_history_export(args: argparse.Namespace) -> int:
    """Export one signed checkpoint-history page; no server contact is made."""
    from .light_client import HISTORY_EXPORT_DEFAULT_LIMIT, export_history

    after = None
    if args.after is not None:
        after = _decimal_int(args.after)
        if after is None:
            return _input_failure()
    limit = HISTORY_EXPORT_DEFAULT_LIMIT
...
def cmd_header_locators(args: argparse.Namespace) -> int:
    """Generate a fork-location request from an advance_headers checkpoint.

    Purely offline: it strictly replays the checkpoint file and prints only
    the generated ``{"locators", "limit"}`` request document as one JSON
    line (contract key order preserved), exiting 0. Any failure prints the
    single-line ``{"ok": false, "error": ...}`` envelope and exits 1: a bad
    ``--limit`` or path is ``input``, a missing/unreadable file ``io`` and
    a corrupt or non-replaying checkpoint ``state``.
    """
    from .light_client import header_locators
    ...
    if args.limit is not None:
        ...
        if limit is None:
            return _input_failure()
    result = header_locators(args.path, limit)
    if result.get("ok") is not True:
        return _emit_result_document(result)
    print(json.dumps(result["request"], sort_keys=False, ensure_ascii=False))
    return 0
...
def cmd_verify_range_batch(args: argparse.Namespace) -> int:
    """Offline verification of a batch of exported incremental range pages.

    Reads a JSON array of range-export documents (the
    ...
    every page against the local trust document. The pages must chain
    continuously (each page's anchor is the previous page's tip), tx_ids must
    be unique across the batch and a pending block may only appear on the last
    page. Prints one JSON line in the verifier's fixed key order and exits 0
    on success, 1 on any failure; unreadable files, non-JSON input and illegal
    anchor arguments all report ``{"ok": false, "error": "input"}``.
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_sync_export = sub.add_parser(
        "sync-export",
        help="export one received sync candidate by source, request_id and mode",
    )
    ...
    p_sync_range_export = sub.add_parser(
        "sync-range-export",
        help="export one received incremental range delivery by source, "
        "request_id and mode",
    )
    ...
    p_verify_range_batch = sub.add_parser(
        "verify-range-batch",
        help="offline-verify an ordered batch of incremental range deliveries "
        "as one continuous chain against a pinned anchor and a trust document",
    )
    p_verify_range_batch.add_argument(
        "--exports",
        required=True,
        metavar="FILE|-",
        help="JSON array of range-export documents in delivery order, or - "
    ...
    p_history_trust.add_argument(
        "path", help="signer log file path (written with compact JSON + newline)"
    )
    p_history_trust.add_argument(
        "--root-key",
        help="64 lowercase hex Ed25519 seed of the log root key",
    )
    p_history_trust.add_argument(
        "--at",
        help="entry instant: a positive decimal integer (update field)",
    )
    p_history_trust.add_argument(
        "--key",
        help="64 lowercase hex Ed25519 public key named by the entry",
    )
    p_history_trust.add_argument(
        "--status",
        help="entry status: active or revoked (update field); all four "
        "options update, none reads",
    )
    p_history_trust.set_defaults(func=cmd_history_trust)

    p_history_export = sub.add_parser(
        "history-export",
        help="export one signed checkpoint-history page (offline)",
    )
    p_history_export.add_argument(
        "path", help="advance checkpoint path (its .history sidecar is read)"
    )
    p_history_export.add_argument(
        "--key",
        required=True,
    ...
    p_history_export.add_argument(
        "--after",
        help="cursor generation (non-negative decimal); omitted starts at base",
    )
    p_history_export.add_argument(
        "--limit",
        help="page size (decimal, 1-200, default 50)",
    )
    p_history_export.add_argument(
        "--trust",
        help="optional durable signer log (see history-trust); pages are cut "
    ...
    p_header_locators = sub.add_parser(
        "header-locators",
        help="build a fork-location request by replaying a header checkpoint",
    )
    p_header_locators.add_argument(
        "path",
        help="advance_headers checkpoint file path (the file is only read)",
    )
    p_header_locators.add_argument(
        "--limit",
        help="request page size carried into the generated request "
        "(decimal, 1-500, default 100)",
    )
