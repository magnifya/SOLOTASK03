...
state-root,
state-proof, state-proofs, confirm, rollback, status, candidates, chain, adopt,
export,
...
def cmd_state_proofs(args: argparse.Namespace) -> int:
    ...
    # (400 on error). The success document has a contract-fixed key order, so it
    # is printed in insertion order rather than alphabetically (error bodies
    # are single-key).
