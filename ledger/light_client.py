...
with :func:`verify_header_locator_pages` against the caller's ``locators``
and pinned ``tip_hash``; the checkpoint at ``path`` must already exist (a
...
:func:`reorg_headers`. ``apply_finality(path, document, trust)`` takes no
default arguments: ``path`` must be a non-empty string; ``document`` is
the decoded response with the exact top-level key order
``finalized, tip, auth`` — ``finalized`` a closed ``{height, block_hash}``
(key order ``height, block_hash``, hash 64 lowercase hex), ``tip`` the
chain descriptor S and ``auth`` ``{key_version, signature}`` (a
non-boolean positive version and a 128-char lowercase hex signature);
``trust`` must carry ``audit_signers`` exactly as for
:func:`verify_header_page`. Structural and type defects are ``input``.
The checkpoint is then strictly reloaded and replayed under the shared
per-path lock (a missing file is ``io``; a parse, key-order, digest or
replay defect is ``state``) before the credential is judged: an unknown
key version or a failed Ed25519 verification over
``SHA256(UTF8("ledger-finality-v1") || canonical_json(document without
auth))`` is ``auth``; afterwards the credential's ``tip`` must be
field-for-field identical to the locally replayed tip, and its
``finalized`` target must name that branch's anchor or one of its
confirmed headers and must not move the stored boundary backwards or
sideways (a mismatched tip, an unknown, pending or dropped-fork target,
or a regressing boundary is ``integrity``). Applying the exact stored
boundary is idempotent — it returns the current generation and leaves
the file bytes untouched; raising the boundary increments the
generation once and atomically rewrites the file in the version-3
format (same serialization and hash rules as
:func:`finalize_headers`). Success returns
``{"ok": True, "generation", "finalized"}`` in that key order; failure
...
tip_hash, trust)`` verifies one signed header-page batch exactly as
:func:`advance_headers` does (same ``documents``/``anchor``/``tip_hash``/
``trust`` arguments, same first-use checkpoint-opening rules — a missing
file is opened with the caller-pinned ``anchor``) and, in the same locked
transaction, applies a non-empty, ordered array of signed finality
credentials kept under the exact :func:`apply_finalities` contract (key
order ``finalized, tip, auth``, nested types, the ``ledger-finality-v1``
signature against ``trust.audit_signers``). Processing is staged:
structure and types first (``input``); then the checkpoint is strictly
loaded and replayed under the shared per-path lock (a parse, digest or
replay defect is ``state``, a read/write failure ``io``); then every
credential envelope is authenticated in array order (an unknown version
or a bad signature is ``auth``); finally the integrity rules run against
the freshly advanced branch — each credential's ``tip`` must match the
same-height header in hash and status, tip heights must not decrease,
finalized heights must strictly increase, and each target must name the
checkpoint anchor or a confirmed header no higher than its own tip. The
batch as a whole may not end below or beside the stored boundary. Only a
fully valid call appends exactly one ``linear`` step and writes the last
credential's boundary, with the generation incremented exactly once and
the file atomically replaced in the version-3 format. A call whose last
step and final boundary both equal the stored ones is idempotent: the
file bytes stay exactly as they were and the generation holds. Success
returns ``{"ok": True, "generation", "tip", "finalized", "applied"}`` in
that key order with ``applied`` the number of credentials; failure
...
:func:`reorg_finalized_headers` is the atomic combination of
:func:`reorg_headers` and :func:`apply_finalities`:
``reorg_finalized_headers(path, documents, finalities, locators,
tip_hash, trust)`` verifies one signed locator-page batch exactly as
:func:`reorg_headers` does (same ``documents``/``locators``/``tip_hash``/
``trust`` arguments and the same existing-checkpoint requirement — a
missing file is ``io``, there is no first use) and, in the same locked
transaction, applies a non-empty, ordered array of signed finality
credentials kept under the exact :func:`apply_finalities` contract (key
order ``finalized, tip, auth``, nested types, the ``ledger-finality-v1``
signature against ``trust.audit_signers``). Processing is staged:
structure and types first (``input``); then the checkpoint is strictly
loaded and replayed under the shared per-path lock (a parse, digest or
replay defect is ``state``, a read/write failure ``io``); every
credential envelope is then authenticated in array order (an unknown
version or a bad signature is ``auth``); finally the integrity rules run
— the reorg boundary is the :func:`reorg_headers` boundary and may never
be earlier than the stored ``finalized`` boundary, the steps after it are
dropped and the new ``locator`` step forms the reorged branch, and each
credential's ``tip`` must match that branch's same-height header in hash
and status, tip heights must not decrease, finalized heights must
strictly increase, and each target must name the anchor or a confirmed
header no higher than its own tip; the last target may not move the old
stored boundary backwards or sideways. A different batch appends the one
locator step and writes the last credential's boundary, with the
generation incremented exactly once. When the submitted locator step is
the stored last step it is not appended again: with the final boundary
unchanged the call is idempotent (the file bytes stay exactly as they
were and the generation holds); with a different final boundary only the
boundary is advanced and ``replaced`` is 0. Success returns
``{"ok": True, "generation", "tip", "finalized", "replaced", "applied"}``
in that key order with ``replaced`` the dropped step count and
``applied`` the number of credentials; failure returns only
``{"ok": False, "error": category}`` with category one of
``input/auth/integrity/state/io``, nothing is raised and a failed call
never changes the file bytes or the generation.
...
# Domain separator prefixing every signed finality credential.
FINALITY_DOMAIN = "ledger-finality-v1"

# The exact contract key order of a signed finality credential and of its
# nested documents (mirrors GET /v1/chain/finality).
FINALITY_KEYS = ("finalized", "tip", "auth")
FINALITY_AUTH_KEYS = ("key_version", "signature")

# Note: the success document of :func:`apply_finality` reuses
# FINALIZE_HEADERS_RESULT_KEYS (``ok, generation, finalized``).


def _finality_bytes(unsigned: dict) -> bytes:
    """The signed bytes of a finality credential without its ``auth`` envelope:
...
def sign_finality(
    ...
    """Build the ``{key_version, signature}`` envelope of a finality credential.
    ...
    digest = hashlib.sha256(_finality_bytes(unsigned)).digest()
...
# kind and the locator request a reorg batch was verified against.
HEADER_STEP_KEYS = ("tip_hash", "trust", "documents")
HEADER_STEP_V2_KEYS = ("kind", "tip_hash", "trust", "documents", "locators")

# The stored step kinds: ``linear`` (an advance_headers batch, locators
...
def _header_tip_reorgs(stored: dict, new: dict) -> bool:
    """Whether ``new`` is a legal reorg successor tip of ``stored``.

    The tip may never drop in height; at the same height a different
    ``block_hash`` — the reorganization itself — is accepted in either
    status, while the same hash may only move from ``pending`` to
    ``confirmed``. Anything else at the same height is a conflict.
    """
    if new["height"] != stored["height"]:
        return new["height"] > stored["height"]
...
def _validate_header_checkpoint_shape(data: object) -> dict:
    ...
    for raw in steps:
        if not isinstance(raw, dict):
            raise _CheckpointError(ERR_STATE)
        if v == 1:
            # Legacy steps carry only the linear-batch fields; they replay
            # as linear steps with no locator request.
            if tuple(raw.keys()) != HEADER_STEP_KEYS:
                raise _CheckpointError(ERR_STATE)
            kind = "linear"
            locators = None
        else:
            if tuple(raw.keys()) != HEADER_STEP_V2_KEYS:
                raise _CheckpointError(ERR_STATE)
            kind = raw["kind"]
            ...
            if kind == "linear":
                # A linear step never carries a locator request.
                if locators is not None:
                    raise _CheckpointError(ERR_STATE)
            elif not isinstance(locators, list) or not locators:
                raise _CheckpointError(ERR_STATE)
...
def reorg_headers(
    ...
    """Verify a signed locator-page batch and durably checkpoint the reorg.
    ...
    and the pinned ``tip_hash``. The checkpoint at ``path`` must already
    exist — a reorg rewrites stored history, so a missing file is an
    ``io`` failure and there is no first use.
    ...
    with lock:
        ...
        if step == steps[-1]:
            # Re-submitting the exact last locator batch (same tip_hash,
            # trust, documents and locators) is idempotent: it was verified
            # when it was stored and re-verified by the load replay above,
            # so the file bytes stay exactly as they were, the generation
            # holds and nothing was replaced.
            return {
                "ok": True,
        ...
        try:
            ...
            # The boundary is the closing tip of the last step matching the
            # batch's anchor; only when no step matches may the checkpoint's
            # initial anchor bound the reorg.
...
# -- signed finality credentials (GET /v1/chain/finality) ----------------------
...
def _parse_finality_document(document: object) -> tuple[dict, dict, int, str]:
    ...
    Every defect here is an input error. Returns the closed finalized
    boundary, the closed tip descriptor, the envelope's key version and its
    signature.
    """
    if not isinstance(document, dict) or tuple(document.keys()) != FINALITY_KEYS:
        raise _Failure(ERR_INPUT)
...
def _authenticate_finality(
    document: dict, key_version: int, signature: str, signers: dict
) -> None:
    """Stage 2 of the finality contract: signer lookup and signature.

    Mirrors :func:`_authenticate_header_page`: the envelope's
    ``key_version`` selects the public key from ``trust.audit_signers``; an
    unknown version or an Ed25519 signature that does not verify over the
    domain-prefixed canonical bytes of the document without its ``auth`` is
    an auth failure.
    """
    public_key = signers.get(key_version)
    if public_key is None:
        raise _Failure(ERR_AUTH)
    unsigned = {key: document[key] for key in FINALITY_KEYS if key != "auth"}
    digest = hashlib.sha256(_finality_bytes(unsigned)).digest()
    if not crypto.verify_signature(public_key, digest, signature):
        raise _Failure(ERR_AUTH)


def apply_finality(path: object, document: object, trust: object) -> dict:
    """Apply a signed ``GET /v1/chain/finality`` credential to a checkpoint.

    The credential document has the exact top-level key order
    ``finalized, tip, auth``: ``finalized`` the irreversible boundary as
    ``{height, block_hash}``, ``tip`` the chain descriptor S and ``auth``
    the ``{key_version, signature}`` envelope signed under the
    ``ledger-finality-v1`` domain by the version selected from
    ``trust.audit_signers``.

    The checkpoint at ``path`` is strictly loaded and fully replayed under
    the per-path lock shared with every other same-path operation (a
    missing file is ``io``; a parse, key-order, digest or replay defect is
    ``state``) — persisted state is settled before the credential is
    judged. Authentication follows: the envelope's ``key_version`` must
    resolve in ``trust.audit_signers`` and its Ed25519 signature must
    verify under the ``ledger-finality-v1`` domain (an unknown version or
    a failed signature is ``auth``). The credential's ``tip`` must then be
    field-for-field identical to the locally replayed tip; the finalized
    target must name the replayed branch's anchor or one of its confirmed
    headers and may not move the stored boundary backwards or sideways. A
    credential naming the exact stored boundary is idempotent (the file
    bytes are untouched and the generation holds); a higher boundary
    increments the generation once and atomically rewrites the file in
    the version-3 format.

    Success returns ``{"ok": True, "generation", "finalized"}`` in that
    key order. Failure returns only ``{"ok": False, "error": category}``
    with category one of ``input`` (argument/document/trust shape or
    types), ``auth`` (unknown key version or a failed signature),
    ``integrity`` (tip, branch-ownership or boundary-regression defects),
    ``state`` (an existing checkpoint fails its parse, key-order, digest
    or replay checks) and ``io`` (the checkpoint is missing or cannot be
    read or written; a failed write restores the original bytes
    best-effort). Nothing is raised and a failed application never
    ...
    lock = _checkpoint_lock(path)
    with lock:
        # Stage 1 (input): exact document/trust key order and raw types.
        # Structural argument validation precedes any file work, exactly as
        # finalize_headers validates its arguments before loading.
        try:
            target, cred_tip, key_version, signature = _parse_finality_document(
                document
            )
            signers = _validate_header_trust(trust)
        except _Failure as failure:
        ...
        # before the credential is judged: a corrupt or missing file is
        # state/io regardless of the credential it arrives with (mirrors
        # advance_headers/reorg_headers/finalize_headers and the
        # load-before-authenticate ordering of history_trust).
        try:
            loaded = _load_header_checkpoint_document(path)
        except _CheckpointError as failure:
            return _failed_advance(failure.category)
        ...
        # Stage 2 (auth): signer lookup and the domain signature.
        try:
            _authenticate_finality(document, key_version, signature, signers)
        except _Failure as failure:
            return _failed_advance(failure.category)

        # Stage 3 (integrity): the credential must describe exactly the
        # locally replayed state.
        if cred_tip != checkpoint["tip"]:
            return _failed_advance(ERR_INTEGRITY)

        finalized = checkpoint["finalized"]
        if target == finalized:
            # The exact current boundary replayed: idempotent, the file
        ...
        # The target must be a confirmed header of the replayed current
        # branch (or its anchor). An unknown height (including a height
        # past the tip), a dropped-fork hash or a pending header cannot be
        # finalized.
        entry = branch.get(target["height"])
...
def advance_finalized_headers(
    ...
    """Advance a header checkpoint and its finality boundary atomically.

    ``documents``/``anchor``/``tip_hash``/``trust`` are exactly the
    :func:`advance_headers` arguments: ``documents`` is an ordered,
    non-empty batch of ``GET /v1/chain/headers`` pages verified with
    :func:`verify_header_pages`; the first use of ``path`` requires a
    legal ``anchor`` while a later call passes ``None`` or the stored
    tip's ``{height, block_hash}``. ``finalities`` is a non-empty,
    ordered array of signed ``GET /v1/chain/finality`` credentials kept
    under the exact :func:`apply_finalities` contract (key order
    ``finalized, tip, auth``, nested types, the ``ledger-finality-v1``
    Ed25519 envelope against ``trust.audit_signers``).

    Processing is staged and all-or-nothing:

    1. **input** — ``path``/``tip_hash``, the non-empty ``documents``
       and ``finalities`` arrays, every credential's exact key order and
       raw types and the trust signers are settled before any file or
       signature work.
    2. **state/io** — the checkpoint at ``path`` is strictly loaded and
       fully replayed under the shared per-path lock (a missing file
       opens a new checkpoint exactly as :func:`advance_headers` does; a
       parse, key-order, digest or replay defect is ``state``).
    ...
    4. **integrity** — the header batch is verified and must legally
       advance the stored tip (the :func:`advance_headers` rules), then
       the credentials walk the freshly advanced branch in order: each
       item's ``tip`` must match the same-height header's hash and
       status, tip heights must not decrease, finalized heights must
       strictly increase, and each target must name the checkpoint
       anchor or a confirmed header no higher than its own tip. The
       batch as a whole may not end below or beside the stored boundary.

    Only a fully valid call appends exactly one ``linear`` step and
    writes the last credential's boundary: the generation is incremented
    exactly once and the file atomically replaced in the version-3
    format (same serialization and hash rules as
    :func:`advance_headers`). A call whose appended step equals the
    stored last step and whose final boundary equals the stored one is
    idempotent: the file bytes stay exactly as they were and the
    generation holds. Success returns ``{"ok": True, "generation",
    "tip", "finalized", "applied"}`` in that key order with ``tip`` the
    new chain descriptor, ``finalized`` the last credential's closed
    boundary (key order ``height, block_hash``) and ``applied`` the
    number of credentials. Failure returns only ``{"ok": False,
    "error": category}`` with category one of ``input`` (argument, array
    or document shape/types), ``auth`` (an unknown version or a failed
    signature), ``integrity`` (chain, tip, ordering, branch or boundary
    defects), ``state`` (an existing checkpoint fails its parse,
    key-order, digest or replay checks; it is never truncated or
    rebuilt) and ``io`` (a read/write failure; a failed write restores
    the original bytes best-effort). Nothing is raised and a failed call
    never changes the file bytes.
    """
    # Argument shape is validated before any file or verification work.
    if not isinstance(path, str) or not path:
    ...
    lock = _checkpoint_lock(path)
    with lock:
        # Stage 1 (input): the batch shapes, every credential's exact key
        # order and raw types and the trust signers are settled before
        # any file or signature work — structure precedes state and
        # trust.
        if not isinstance(documents, list) or not documents:
            return _failed_advance(ERR_INPUT)
        ...
        # Stage 2 (state/io): the persisted checkpoint is strictly loaded
        # and fully replayed under the shared per-path lock before
        # anything is judged; a first use opens the checkpoint exactly as
        # advance_headers does.
        try:
            loaded = _load_header_checkpoint_document(path)
        except _CheckpointError as failure:
            return _failed_advance(failure.category)
        ...
        if loaded is None:
            # First use: the caller must pin a legal anchor. The anchor
            # is also the initial finalized boundary.
            expected_anchor = _validate_header_anchor_argument(anchor)
            if expected_anchor is None:
                return _failed_advance(ERR_INPUT)
            generation = 0
        ...
        # it was stored and re-verified by the load replay above, so it
        # is neither re-verified nor appended again.
        same_step = bool(steps) and step == steps[-1]

        # Stage 3 (auth): every credential envelope is verified in array
        # order. A later credential is never trusted because an earlier
        # one was.
        try:
            for document, (_target, _tip, key_version, signature) in zip(
                finalities, parsed
        ...
        # The header batch is verified exactly as advance_headers
        ...
        if same_step:
            ...
            try:
                ...
                # Defensive: the verifiers never raise, but an
            ...
            if stored_tip is not None and not _header_tip_advances(
                ...
                return _failed_advance(ERR_INTEGRITY)
            # Extend the replayed branch with the batch's headers so the
            # credentials below are judged against the advanced chain.
            ...
            # The closing tip is authoritative for its height (an empty
            # final page settles the tip's status without headers).
            branch[new_tip["height"]] = (new_tip["tip_hash"], new_tip["status"])

        # Stage 4 (integrity): walk the credential batch in order against
        # the freshly advanced branch, exactly as apply_finalities walks
        # the replayed one. Inside the batch finalized heights rise
        # strictly and tip heights never fall; each item's tip must name
        # the branch header at that height (same hash and status) and its
        # finalized target must be the checkpoint anchor or a confirmed
        # header no higher than that tip.
        previous_target_height: int | None = None
        previous_tip_height: int | None = None
        last_target: dict | None = None
        try:
        ...
        if same_step and last_target == finalized:
            # Idempotent: the appended step and the final boundary both
            # equal the stored ones, so the file bytes stay exactly as
            # they were and the generation holds.
            return {
                "ok": True,
                "generation": generation,
        ...
        next_generation = generation + 1
        new_steps = steps if same_step else steps + [step]
        checkpoint = {
            "v": HEADER_CHECKPOINT_VERSION,
            "generation": next_generation,
            "anchor": file_anchor,
...
def reorg_finalized_headers(
    ...
    """Reorganize a header checkpoint and advance finality atomically.

    :func:`reorg_finalized_headers` is the atomic combination of
    :func:`reorg_headers` and :func:`apply_finalities`:
    ``documents``/``locators``/``tip_hash``/``trust`` are exactly the
    :func:`reorg_headers` arguments — an ordered, non-empty batch of
    ``POST /v1/chain/headers/locate`` pages verified with
    :func:`verify_header_locator_pages`, the caller's ``locators`` and
    the pinned ``tip_hash`` — and the checkpoint at ``path`` must already
    exist (a missing file is ``io``; there is no first use).
    ``finalities`` is a non-empty, ordered array of signed
    ``GET /v1/chain/finality`` credentials kept under the exact
    :func:`apply_finalities` contract (key order ``finalized, tip,
    auth``, nested types, the ``ledger-finality-v1`` Ed25519 envelope
    against ``trust.audit_signers``).

    Processing is staged and all-or-nothing, in the order structure,
    persisted state, signatures, integrity:

    1. **input** — ``path``/``tip_hash`` and the non-empty ``finalities``
       array's exact key order and raw types (plus the trust signers) are
       settled first; the locator batch keeps the
       :func:`reorg_headers` verification contract.
    2. **state/io** — the checkpoint is strictly loaded and fully
       replayed under the shared per-path lock (a missing file is ``io``;
       a parse, key-order, digest or replay defect is ``state``).
    3. **auth** — every credential envelope is verified in array order,
       then the locator batch is authenticated by its own contract (an
       unknown key version or a failed signature is ``auth``).
    4. **integrity** — the reorg boundary is the closing tip of the last
       stored step matching the batch anchor (or the initial anchor when
       no step matches; an anchor anywhere else is ``integrity``) and it
       may never be earlier than the stored ``finalized`` boundary. The
       steps after it are dropped and the one new ``locator`` step forms
       the reorged branch; the new tip follows the
       :func:`reorg_headers` tip rules. The credentials then walk the
       reorged branch in order: each item's ``tip`` must match the
       same-height header in hash and status, tip heights must not
       decrease, finalized heights must strictly increase, and each
       target must name the anchor or a confirmed header no higher than
       its own tip; the last target may not move the old stored boundary
       backwards or sideways.

    A different batch appends the one locator step, writes the last
    credential's boundary and increments the generation exactly once,
    atomically, in the version-3 format. When the submitted locator step
    (same ``tip_hash``, ``trust``, ``documents`` and ``locators``) is the
    stored last step it is neither re-verified nor appended again and
    ``replaced`` is 0: with the final boundary unchanged the call is
    idempotent — the file bytes stay exactly as they were and the
    generation holds — while a different final boundary only advances
    the boundary, once. Success returns ``{"ok": True, "generation",
    "tip", "finalized", "replaced", "applied"}`` in that key order with
    ``replaced`` the dropped step count and ``applied`` the number of
    credentials. Failure returns only ``{"ok": False, "error":
    category}`` with category one of ``input`` (argument, array or
    document shape/types), ``auth`` (an unknown version or a failed
    signature), ``integrity`` (finality, boundary, chain, tip, ordering,
    branch or boundary defects), ``state`` (an existing checkpoint fails
    its parse, key-order, digest or replay checks; it is never truncated
    or rebuilt) and ``io`` (a missing file or a read/write failure; a
    failed write restores the original bytes best-effort). Nothing is
    raised and a failed call never changes the file bytes or the
    generation.
    """
    # Stage 1 (input): argument shape and the whole finality batch's
    # exact key order and raw types, plus the trust signers, are settled
    # before any file or signature work. The locator batch keeps the
    # reorg_headers contract — its structure is judged together with its
    # signatures and chain by verify_header_locator_pages after the
    ...
    with lock:
        ...
        # loaded and fully replayed before any batch or credential is
        # judged — a reorg rewrites an existing checkpoint, so a missing
        # file is io and there is no first use.
        try:
            loaded = _load_header_checkpoint_document(path)
        except _CheckpointError as failure:
            return _failed_advance(failure.category)
        except Exception:
            # Defensive: an unreadable-by-surprise checkpoint must
        ...
        # above, so the locator batch is neither re-verified nor
        # re-appended; the credentials below are still judged.
        same_step = step == steps[-1]

        # Stage 3 (auth): every finality envelope is verified in array
        # order before the locator batch's own authentication runs.
        try:
            for document, (_target, _tip, key_version, signature) in zip(
                finalities, parsed
            ):
        ...
        try:
            if same_step:
                ...
                # matching the batch anchor; only when no step matches
                ...
                # The locator anchor may never be earlier than the
                # stored finalized boundary: finalized history is
                # irreversible.
                if batch_anchor["height"] < finalized["height"] or (
                    batch_anchor["height"] == finalized["height"]
                    and batch_anchor["block_hash"] != finalized["block_hash"]
                ...
                # Form the reorged branch: prune the dropped fork from
                # the replayed current branch at the boundary and record
                # the batch's headers, exactly as the checkpoint replay
                # does for a locator step.
                post_branch = dict(branch)
                boundary_height = batch_anchor["height"]
                for height in [h for h in post_branch if h > boundary_height]:
                    del post_branch[height]
                for document in documents:
                    for header in document["headers"]:
                        post_branch[header["height"]] = (
                            header["block_hash"],
                            header["status"],
                        )
                ...
                # headers).
                post_branch[new_tip["height"]] = (
                    new_tip["tip_hash"],
                    new_tip["status"],
                )
                new_steps = kept_steps + [step]

            # Stage 4 (integrity): walk the credential batch in order
            # against the reorged branch under the apply_finalities
            # rules: tip hash/status at the same height, non-decreasing
            # tip heights, strictly increasing finalized heights, and a
            # target that names the checkpoint anchor or a confirmed
            # header no higher than the credential's own tip.
            previous_target_height: int | None = None
            previous_tip_height: int | None = None
            last_target: dict | None = None
            for target, cred_tip, _version, _signature in parsed:
                tip_entry = post_branch.get(cred_tip["height"])
                if tip_entry is None or tip_entry[0] != cred_tip["tip_hash"]:
                    raise _Failure(ERR_INTEGRITY)
                if tip_entry[1] != cred_tip["status"]:
                    raise _Failure(ERR_INTEGRITY)
                ...
                if target["height"] > cred_tip["height"]:
                    raise _Failure(ERR_INTEGRITY)
                entry = post_branch.get(target["height"])
                if entry is None or entry[0] != target["block_hash"]:
                    raise _Failure(ERR_INTEGRITY)
                if (
                    target != checkpoint["anchor"]
        ...
        if same_step:
            if last_target == finalized:
                # Idempotent: the stored last locator step and the final
                # boundary both equal the submitted ones, so the file
                # bytes stay exactly as they were, the generation holds
            ...
            # Same step, higher boundary: only the boundary advances,
            # once, with no step appended and nothing replaced.
            next_generation = checkpoint["generation"] + 1
            rewritten = {
                "v": HEADER_CHECKPOINT_VERSION,
                "generation": next_generation,
            ...
            # A genuine reorg always appends the one locator step and
            # writes the last credential's boundary, one generation.
            next_generation = checkpoint["generation"] + 1
            rewritten = {
                "v": HEADER_CHECKPOINT_VERSION,
                "generation": next_generation,
