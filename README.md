...
- **过期处理**：同步带来的候选只在记录未过期期间存活；一旦过期，记录与其候选
  ...
  **并按当前信任注册表重新校验每条记录的来源授权**——来源未知、已撤销或注册已
  过期的记录连同其候选一并丢弃（已采用 tip 仅保留审计历史），canonical 链不变。
  **停机补写**：重启恢复逐条核对指纹、摘要、期限与当前授权。对于在停机期间
  「请求到期」或「来源失效（未知/已撤销/注册过期）」的记录——即节点若在线本会由
  惰性清理写出 `sync_expired` 的那些——为**每条**补写且只补写一个 `sync_expired`
  事件，`event_id` 从 1 起连续、字段与运行时清理完全一致；已存在等价事件（同
  source/request_id/tip_hash/expires_at）或已采用 tip 的记录不重复补写，已采用
  tip 绝不改变 canonical，仅移除到期元数据并保留 sync_received、sync_adopted、
  sync_expired 历史。而**完整性失效**（结构损坏、tip 无处解析、指纹/摘要不符）
  的记录不是运行时清理能产生的情形，仍静默丢弃、不补事件。补写与被清理的
  记录/候选在同一次原子写入中落盘并递增 `generation`；写盘失败不改动磁盘，下次
  启动整体重试，事件不丢失、不重复。
