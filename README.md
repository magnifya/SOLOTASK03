...
- **持久化头检查点**：`ledger.light_client.advance_headers(path,
  ...
  `v, generation, anchor, tip, steps, hash`：`v = 2`，`generation` 为非
  布尔正整数、每次成功推进 +1，`anchor` 为首次钉住的锚点，`tip` 沿用链
  描述符 S，`steps` 为非空数组且每项键序恰为
  `kind, tip_hash, trust, documents, locators`（推进追加 `linear` 步，
  其 `locators` 为 null；嵌套键序沿用既有契约；v1 三键步文件仍可读
  写时升级），`hash` 为去掉
...
- **检查点重组**：`ledger.light_client.reorg_headers(path, documents,
  locators, tip_hash, trust) -> dict` 用既有 `verify_header_locator_pages`
  多页定位验证器验批后重写头检查点（纯库 API，HTTP/CLI 不变）。边界为
  匹配锚点（批次首页锚，必为 `locators` 之一）所命中的**最后一个**
  step 的 tip；无任何 step 匹配时才取初始锚点，两者皆不命中为
  `integrity`。删去边界之后的全部 step（删除数即 `replaced`），追加一
  个 locator 步。新 tip 高度不得低于已存 tip；同高度异 hash 时
  pending/confirmed 均可，同 hash 仅允许 `pending`→`confirmed`，其余为
  同高冲突（`integrity`）。末步为与新步全同的 locator 步时幂等：
  `replaced = 0`、不写盘、不增代；其余成功 `generation + 1`。检查点要
  求已存在（缺失为 `io`）。文件格式即 v2：顶层键序
  `v, generation, anchor, tip, steps, hash`，`v = 2`，步键序
  `kind, tip_hash, trust, documents, locators`，`kind` 取
  `linear`（`locators` 为 null）或 `locator`；v1 文件（三键步，一律按
  `linear` 读）仍可加载重放，下次成功写入即升级为 v2，
  `advance_headers` 亦读两版、写 v2 并追加 linear 步。hash 与序列化约
  定不变。成功按键序 `ok, generation, tip, replaced` 返回；失败仅
  `{"ok": false, "error"}` 且不抛异常，`error` 取 `input`（结构）、
  `auth`（密钥/验签）、`integrity`（边界/链/高度或同高冲突）、
  `state`（存量文件损坏）、`io`（文件缺失或读写失败）；同 path 串行，
  失败不改字节与代数。
